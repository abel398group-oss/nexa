...
interface ContactModalState {
  step: WizardStep;
  editId: string | null;
  /** T10-WIZARD (2026-07-17): canal principal do contato — passo 1 começa
   *  escolhendo entre WhatsApp e e-mail. 'whatsapp' mantém os campos de sempre
   *  (WhatsApp + e-mails); 'email' esconde o WhatsApp e cadastra só por e-mail. */
...
export function MonitorConfigPage() {
  ...
  /** T10-WIZARD: troca o canal principal no passo 1. Ao ir pra e-mail, limpa o
   *  WhatsApp (não conta como canal); ao voltar pro WhatsApp, mantém o resto. */
  function setContactChannel(channel: 'whatsapp' | 'email') {
    setContactModal((m) =>
      m ? { ...m, channel, whatsapp: channel === 'email' ? '' : m.whatsapp, error: null } : m,
    );
  }
  ...
  function validateStep1(m: ContactModalState): string | null {
    // T10-WIZARD: ramo e-mail — exige ao menos um e-mail, ignora WhatsApp.
    if (m.channel === 'email') {
      if (m.emails.length === 0) return 'Informe pelo menos um e-mail.';
      return null;
    }
      ...
      >
        {contactModal && (
          <div className="space-y-6">
            ...
            {contactModal.step === 1 && (
              <div className="space-y-6">
                {/* T10-WIZARD: seletor de canal — WhatsApp (campos de sempre) ou e-mail (só nome + e-mail). */}
                <div>
                  <p className="text-sm font-medium text-base-content/70 mb-1.5">Como esse contato recebe?</p>
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="inline-flex overflow-hidden rounded-md border border-base-300">
                      <button
                        type="button"
                        aria-pressed={contactModal.channel === 'whatsapp'}
                        onClick={() => setContactChannel('whatsapp')}
                        className={`px-4 py-2 text-sm font-medium transition-colors ${
                          contactModal.channel === 'whatsapp' ? 'bg-brand-500 text-white' : 'text-base-content/60 hover:bg-base-200'
                        }`}
                      >
                        📱 WhatsApp
                      </button>
                      <button
                        type="button"
                        aria-pressed={contactModal.channel === 'email'}
                        onClick={() => setContactChannel('email')}
                        className={`border-l border-base-300 px-4 py-2 text-sm font-medium transition-colors ${
                          contactModal.channel === 'email' ? 'bg-brand-500 text-white' : 'text-base-content/60 hover:bg-base-200'
                        }`}
                      >
                        ✉️ E-mail
                      </button>
                ...
                {contactModal.channel === 'whatsapp' ? (
                    ...
                    <div>
                      ...
                      {atWaLimit && !contactModal.startedWithWa && (
                        <p className="mt-1.5 text-[11px] text-warning">
                          Limite de números do plano atingido — troque o canal para e-mail (ilimitado).{' '}
                          <a
                            href="https://app.hipertms.com.br/configuracoes/assinatura"
                            target="_blank"
                            rel="noopener noreferrer"
                  ...
                  <div>
                    <RecipientTagsInput
                      ...
                      max={999}
                    />
                    {/* T10-WIZARD: dedup — avisa quando o e-mail já existe em outro contato (não repetir). */}
                    {(() => {
                      const dup = contacts.find(
                        (c) =>
                          c.id !== contactModal.editId &&
                          c.emails.some((e) => contactModal.emails.some((me) => me.toLowerCase() === e.toLowerCase())),
                      );
                      return dup ? (
                        <div className="mt-2 flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/5 px-3 py-2">
                          ...
                          <p className="text-xs text-warning">
                            ...
                            <strong className="font-medium">{dup.name || dup.whatsapp || dup.emails[0]}</strong>.{' '}
                            Edite aquele contato em vez de criar um novo pra não duplicar o envio.
                          </p>
                        </div>
                      ) : null;
                    })()}
                ...
                {planAllowed && waNumbersLimit > 0 && contactModal.channel === 'whatsapp' && (
