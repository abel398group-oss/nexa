...
// Pagination props adapted to the Nexa Pagination component API (page/pageCount).
...
interface StandardListPageProps {
  /** Ícone opcional no header (componente SVG que aceita className) */
  icon?: React.ComponentType<{ className?: string }>;
  iconColor?: string;
  title: string;
  breadcrumb?: BreadcrumbItem[];
  description?: string;
  /** Contagem total de registros no servidor */
  totalItems?: number;
  /** Quantos estão sendo mostrados nesta página */
  totalShowing?: number;
  /** Rótulo no plural para o tipo de registro (ex: "contatos") */
  entityName?: string;
  isLoading?: boolean;
  /** true quando já há dados em cache (evita tela de loading no refetch) */
  hasData?: boolean;
  error?: unknown;
  onRetry?: () => void;
  ...
  /** Botões no lado direito do header (ex: "+ Novo Contato") */
  headerActions?: React.ReactNode;
  /** Controles de filtro extra na toolbar */
  filtersContent?: React.ReactNode;
  /** Ações em lote / contextuais na toolbar */
  actionsContent?: React.ReactNode;
  /** Qualquer conteúdo adicional na toolbar */
...
export function StandardListPage({
  ...
  // Guard: loading sem dados em cache — mostra tela cheia de loading
  ...
  // Guard: erro sem dados — mostra tela de erro com retry
  if (error && !hasData) {
    return (
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
          <div>
            ...
            {countLabel && (
              <p className="mt-1 text-sm text-base-content/60">{countLabel}</p>
            )}
          </div>
          {headerActions && (
            <div className="flex shrink-0 gap-2">{headerActions}</div>
          )}
        </div>
        ...
        {/* Cartão principal com tabela + paginação */}
