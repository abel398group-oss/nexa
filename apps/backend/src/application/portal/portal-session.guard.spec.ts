...
describe('PortalSessionGuard', () => {
  it('sem cookie nem bearer -> 401', async () => {
  ...
  it('cookie valido -> popula req.portalCustomer', async () => {
  ...
  it('Authorization Bearer valido (sem cookie) -> popula req.portalCustomer', async () => {
    const customer = { externalId: 'e', tenantId: 't', name: 'A' };
    const req: any = { headers: { authorization: 'Bearer good' } };
    const g = new PortalSessionGuard({ verify: vi.fn().mockResolvedValue(customer) } as any);
    expect(await g.canActivate(ctx(undefined, req))).toBe(true);
    expect(req.portalCustomer).toEqual(customer);
  });
});
